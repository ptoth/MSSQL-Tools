...
--Databases users and rolesDECLARE @SQLStatement VARCHAR(4000)DECLARE @T_DBuser TABLE (dbname SYSNAME, username SYSNAME, associateddbrole NVARCHAR(256))SET @SQLStatement='
...
WHERE dp.sid NOT IN (0x01) AND dp.sid IS NOT NULL AND dp.type NOT IN (''C'') AND dp.is_fixed_role <> 1 AND dp.name NOT LIKE ''##%'' AND ''?'' NOT IN (''master'',''msdb'',''model'',''tempdb'') ORDER BY DBName'INSERT @T_DBuser
EXEC Sp_msforeachdb @SQLStatementSELECT * FROM @T_DBuser ORDER BY dbname

--Get objects permission of specified user database USE <DATABASE NAME>goDECLARE @Obj VARCHAR(4000)DECLARE @T_Obj TABLE (username SYSNAME, objectname SYSNAME, permission NVARCHAR(128))SET @Obj='
...
ON dp.major_id = Obj.id 'INSERT @T_Obj
EXEC Sp_msforeachdb @ObjSELECT * FROM @T_Obj