	...
	WHERE LOWER(sd.[name]) NOT IN (''master'', ''tempdb'', ''model'', ''reportservertempdb'',''semanticsdb'')
		...
		AND is_distributor = 0)
			...
			(
				...
				/* Retrieve the list of databases to loop, excluding AlwaysOn secondary replicas */
...
BEGIN SET @hasIXsOUT = 1 END ELSE BEGIN SET @hasIXsOUT = 0 END'
		...
		SELECT @startDateTime = GETDATE(), @endDateTime = DATEADD(minute, @timeLimit, GETDATE()), @operationFlag = NULL, @ver = '1.6.1';
...
Rebuild columnstore indexes with fragmentation greater than ' + CAST(@rebuildThreshold_cs AS VARCHAR(10)) + ';
	...
	AND t.name + ''.'' + mst.name = ''' + @tblName + ''';' ELSE ';' END
				...
				BEGIN
					...
					BEGIN CATCH
						IF @debugMode = 1
						BEGIN
							SET @debugMessage = '     Error ' + CONVERT(VARCHAR(20),ERROR_NUMBER()) + ' has occurred while determining which rowstore indexes to defragment. Message: ' + ERROR_MESSAGE() + ' (Line Number: ' + CAST(ERROR_LINE() AS VARCHAR(10)) + ')'
							RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN TRY
							SELECT @ColumnStoreGetIXSQL = 'SELECT @dbID_In, DB_NAME(@dbID_In), rg.object_id, rg.index_id, rg.partition_number, SUM((ISNULL(rg.deleted_rows,1)*100)/rg.total_rows) AS [fragmentation], SUM(ISNULL(rg.size_in_bytes,1)/1024/8) AS [simulated_page_count], rg.total_rows, GETDATE() AS [scanDate]
...
GROUP BY rg.object_id, rg.index_id, rg.partition_number, rg.total_rows
OPTION (MAXDOP 2)'
						...
						BEGIN CATCH
							IF @debugMode = 1
							BEGIN
								SET @debugMessage = '     Error ' + CONVERT(VARCHAR(20),ERROR_NUMBER()) + ' has occurred while determining which columnstore indexes to defragment. Message: ' + ERROR_MESSAGE() + ' (Line Number: ' + CAST(ERROR_LINE() AS VARCHAR(10)) + ')'
								RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
								--RAISERROR('     An error has occurred executing the pre-command! Please review the tbl_AdaptiveIndexDefrag_log table for details.', 0, 42) WITH NOWAIT;
							END
						END CATCH
...
WHERE ids.[dbID] = ' + CAST(@dbID AS VARCHAR(10));
...
AND (si.[type] IS NULL OR si.[type] NOT IN (5,6,7))' -- Avoid error 35337
		...
		IF @debugMode = 1
		SELECT @debugMessage = 'Looping through batch list... There are ' + CAST(COUNT(DISTINCT indexName) AS VARCHAR(10)) + ' indexes to defragment in ' + CAST(COUNT(DISTINCT dbName) AS VARCHAR(10)) + ' database(s)!'
		...
		BEGIN
			...
			BEGIN
				SELECT @debugMessage = 'Looping through batch list... There are ' + CAST(COUNT(DISTINCT statsName) AS VARCHAR(10)) + ' index related statistics to update in ' + CAST(COUNT(DISTINCT idss.dbName) AS VARCHAR(10)) + ' database(s)!'
			...
			BEGIN
				...
				SELECT @debugMessage = @debugMessage + ' plus ' + CAST(COUNT(DISTINCT statsName) AS VARCHAR(10)) + ' other statistics to update in ' + CAST(COUNT(DISTINCT dbName) AS VARCHAR(10)) + ' database(s)!'
				FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss
...
'AND i.object_id = ' + CAST(@objectID AS VARCHAR(10)) + ' AND i.index_id = ' + CAST(@indexID AS VARCHAR(10)) + ';'
			...
			IF @debugMode = 1
			BEGIN
				SELECT @debugMessage = '    Found ' + CONVERT(VARCHAR(10), @fragmentation) + ' percent fragmentation on index ' + @indexName + '...';
				RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
			END
			...
			BEGIN
				...
				/* Set update statistics command for index only, before rebuild, as rebuild performance is dependent on statistics
			...
			BEGIN
				...
				IF @debugMode = 1
				BEGIN
					SET @debugMessage = '   Determining modification row counter for statistic ' + @statsName + '...';
					RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
				END
				...
				IF @debugMode = 1
				SELECT @debugMessage = '     Found a row modification counter of ' + CONVERT(VARCHAR(10), @rowmodctr) + ' and ' + CONVERT(VARCHAR(10), @record_count) + ' rows...';

				IF @debugMode = 1
				RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;

				/* Because we are reorganizing, we will update statistics if they have changed since last update with customized, more finer values, just like when TF2371 is enabled for Auto-Update.
				...
				*/
				...
				BEGIN
					IF @debugMode = 1
					BEGIN
						SET @debugMessage = '     No need to update statistic ' + @statsName + '...';
						RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
					END
		...
		BEGIN
			...
			BEGIN
				...
				IF @debugMode = 1
				SELECT @debugMessage = '     Found a row modification counter of ' + CONVERT(VARCHAR(10), @rowmodctr) + ' and ' + CONVERT(VARCHAR(10), @record_count) + ' total rows...';
